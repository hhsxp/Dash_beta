...
initial_data_path = "/home/ubuntu/processed_ticket_data.csv" # Fallback data
UPLOAD_DIRECTORY = "/home/ubuntu/dashboard_app/uploads" # Not strictly needed if processing in memory
...
pendente_color = "#6c757d" # Grey for pending

plotly_template = "plotly_dark"

# Load initial data or create empty DataFrame
try:
    df_initial = pd.read_csv(initial_data_path,
                           ...
                           low_memory=False)
    # Add necessary time columns if missing from initial file
    if 'Mes_Ano_Criacao' not in df_initial.columns:
        df_initial['Mes_Ano_Criacao'] = df_initial['Criado'].dt.strftime("%Y-%m")
    if 'Trimestre_Criacao' not in df_initial.columns:
        df_initial['Trimestre_Criacao'] = df_initial['Criado'].dt.quarter
    if 'Ano_Criacao' not in df_initial.columns:
        df_initial['Ano_Criacao'] = df_initial['Criado'].dt.year
    print(f"Loaded initial data: {df_initial.shape[0]} rows")
except Exception as e:
    print(f"Error loading initial data: {e}. Using empty DataFrame.")
    df_initial = pd.DataFrame() # Start with empty if initial load fails
...
app.layout = dbc.Container([
    dcc.Store(id='processed-data-store', storage_type='memory'), # Store processed data
    ...
    dbc.Row([
        dbc.Col([
            ...
            dbc.Button("Processar Arquivos Carregados", id="process-button", n_clicks=0, color="primary", className="mt-2"),
...
def update_data_store(n_clicks, piloto_contents, piloto_filename, sla_contents, sla_filename):
    if n_clicks > 0 and piloto_contents and sla_contents:
        ...
        try:
            ...
            if not df_processed.empty:
                ...
                for col in df_processed.select_dtypes(include=['datetime64[ns, UTC]', 'datetime64[ns]']).columns:
                    df_processed[col] = df_processed[col].astype(str)
                # Replace NaT/NaN/inf with None for JSON compatibility
                df_processed = df_processed.replace({pd.NaT: None})
                df_processed = df_processed.replace([np.inf, -np.inf], np.nan).fillna(value=None)

                status_message = dbc.Alert(f"Arquivos '{piloto_filename}' e '{sla_filename}' processados com sucesso! {df_processed.shape[0]} linhas.", color="success")
                return df_processed.to_dict('records'), status_message
            else:
                status_message = dbc.Alert("Falha no processamento dos arquivos. Verifique os logs ou o formato dos arquivos.", color="danger")
                return dash.no_update, status_message # Keep existing data if processing fails
        except Exception as e:
            print(f"Error during file processing callback: {e}")
            status_message = dbc.Alert(f"Erro ao processar arquivos: {e}", color="danger")
...
# Populate Filter Options based on Data Store
@callback(
...
def update_filter_options(data):
    if not data:
        # Use initial data if store is empty
        df = df_initial.copy()
        if df.empty:
             # Still empty, provide default empty options
        ...
        if 'Criado' in df.columns:
            df['Criado'] = pd.to_datetime(df['Criado'], errors='coerce')
             ...
             # Handle case where 'Criado' might be missing after processing error
             return ([{'label': 'N/A', 'value': 'all'}], 'all', 
                     [{'label': 'N/A', 'value': 'all'}], 'all', 
                     [], None)

    # Project options
    projects = sorted(df['Projeto'].unique()) if 'Projeto' in df.columns else []
    project_options = [{'label': 'Todos', 'value': 'all'}] + [{'label': proj, 'value': proj} for proj in projects]
    default_project = 'all'

    # Tribo options
    tribos = sorted(df['Unidade de Negócio'].unique()) if 'Unidade de Negócio' in df.columns else []
    tribo_options = [{'label': 'Todas', 'value': 'all'}] + [{'label': tribo, 'value': tribo} for tribo in tribos]
    default_tribo = 'all'

    # Period value options (depends on the selected period type - handled in another callback)
    # For now, just return the project/tribo options
    # We'll populate period values dynamically based on period type selection
...
# Populate Period Value Dropdown based on Period Type
@callback(
...
def update_period_value_options(period_type, data):
    if not data:
        ...
        if 'Criado' in df.columns:
             ...
             return [], None # Cannot generate periods without 'Criado' date

    options = []
    default_value = None

    if period_type == 'year':
        years = sorted(df['Ano_Criacao'].unique(), reverse=True) if 'Ano_Criacao' in df.columns else []
        options = [{'label': str(y), 'value': y} for y in years if pd.notna(y)]
    elif period_type == 'quarter':
        df['Ano_Trimestre'] = df['Ano_Criacao'].astype(str) + '-Q' + df['Trimestre_Criacao'].astype(str)
        quarters = sorted(df['Ano_Trimestre'].unique(), reverse=True) if 'Ano_Trimestre' in df.columns else []
        options = [{'label': q, 'value': q} for q in quarters if pd.notna(q)]
    elif period_type == 'month':
        months = sorted(df['Mes_Ano_Criacao'].unique(), reverse=True) if 'Mes_Ano_Criacao' in df.columns else []
        options = [{'label': m, 'value': m} for m in months if pd.notna(m)]
...
def render_tab_content(active_tab):
    if active_tab == "tab-visao-geral":
        ...
        return dbc.Row([
            ...
            dbc.Col(dcc.Graph(id='graph-tempo-medio-status'), md=12, className="mt-3"), # Requires status transition data - Placeholder
        ])
...
# Update Graphs and KPIs based on filters and data store
@callback(
    [
     # Visão Geral KPIs
...
def update_dashboard(data, selected_project, selected_tribo, period_type, selected_period_value):
    # Use initial data if store is empty/invalid
    if not data:
        df = df_initial.copy()
        if df.empty:
            # Return empty figures/defaults if no data available at all
    ...
    if selected_period_value:
        if period_type == 'year':
            filtered_df = filtered_df[filtered_df['Ano_Criacao'] == selected_period_value]
        elif period_type == 'quarter':
    ...
    if not sla_res_applicable.empty:
    ...
    status_colors = { # Map categories to consistent colors
        ...
        'Em Progresso': em_risco_color, # Use risk color for in progress
    ...
    if not sla_resp_applicable.empty:
    ...
    # Fig: Tempo Médio por Status (Placeholder - Requires more complex data/logic)
    ...
    # Add annotation indicating placeholder status
